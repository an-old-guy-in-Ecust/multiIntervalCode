...
inline void updateMinMaxNB(vec *&G, int node, int h, unordered_map<int, unsigned> *&hashesNums, queue<int> &candidates, unordered_set<int> &candidatesUniq, int *&hashesMin, int *&hashesMax)
{
    ...
    // hashesMin: RangeCode中的l
    // hashesMax: RangeCode中的r
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        else
        {
            ...
            for (int j = 0; j != G[i].size(); j++)
            {
                ...
                commonNeighbor(G[i], G[n], CN, j + 1, G[i].size(), id2, G[n].size()); //从编号n开始求i与n公共点个数，防止算重复
            }
    ...
    string b = outdirect + outname + "/" + outname + "_id.csv";
    idfile << ":ID" << endl;
    for (int i = 0; i < N; i++)
        idfile << ids[i] << endl;
    ofstream nodefile;
